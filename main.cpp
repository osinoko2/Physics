...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		BSphere.center = ball.position;
		BSphere.radius = ball.radius;
		BSphere.color = ball.color;
		...
		DrawSphere(BSphere, viewProjectionMatrix, viewportMatrix, BSphere.color);
